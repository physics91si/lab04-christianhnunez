...
def phi_approx(n, show_output=True, stream=None):
    ...
    if stream != None and show_output==True:
        stream.write(phi_approx_output_format.format(n, fib_n, fib_nm1, phi))
    return phi
...
def phi_converge(filename=""):
    ...
    i = 3
    phi_old = phi_approx(i - 1, show_output=False, stream=f)
    phi_new = phi_approx(i, show_output=True, stream=f)
